...
def link(directory, sephora_dir='sephora_data.xlsx', clinique_dir='clinique_reviews.xlsx'):
    DIRECTORY = directory
    sephora_df = pd.read_excel(DIRECTORY + sephora_dir)
    clinique_df = pd.read_excel(DIRECTORY + clinique_dir)
    ...
    df.to_excel(DIRECTORY + 'linked_ratio.xlsx', index=False)
    df_unlinked.to_excel(DIRECTORY + 'unlinked_ratio.xlsx', index=False)
